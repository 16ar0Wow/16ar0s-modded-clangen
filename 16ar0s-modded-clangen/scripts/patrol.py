...
class Patrol():
    ...
    def get_possible_patrols(self, current_season, biome, all_clans, game_setting_disaster = game.settings['disasters']):
        ...
        if len(self.patrol_cats) >= 2:
            possible_patrols.extend([
                PatrolEvent(
                    ...
                    50,
                    10,
                    win_skills=['very smart', 'extremely smart', 'prodigy'])
            ])
        ...
        if randint(1, 1) == 1 and self.other_clan is not None:
            ...
            if other_clan_relations > 17:
                ...
                'You run away from the other patrol', 60, 10,
                other_clan,
                win_skills=['great fighter', 'excellent fighter', 'strategist', 'excellent speaker'],
                win_trait = 'fierce',
        ...
        if self.patrol_random_cat is not None and self.patrol_random_cat.status == 'apprentice' and len(
            ...
            possible_patrols.extend([
                ...
                PatrolEvent(
                    ...
                    50,
                    10,
                    win_skills=['very smart', 'extremely smart', 'prodigy', 'extremely good gatherer']),
                PatrolEvent(
                    117,
                    'While helping gathering herbs, r_c stumbles upon a bush of red berries',
    ...
    def add_new_cats(self):
        ...
        if self.patrol_event.patrol_id in [500, 501, 510]:  # new loner
            ...
            new_backstory = choice(['loner1', 'loner2', 'rogue1', 'rogue2', 'cave', 'wanderer1', 'wanderer2', 'wanderer3', 'wanderer4', 
                                    'wanderer5' 'framed_exile', 'ostracized_warrior', 'disgraced', 'retired_leader', 'refugee', 'tragedy_survivor'])
            ...
            new_backstory = choice(['medicine_cat1', 'medicine_cat2', 'disgraced', 'loner1', 'loner2'])
            kit.backstory = new_backstory
            kit.skill = choice(['good healer', 'great healer', 'fantastic healer'])
            kit.thought = 'Is looking around the camp with wonder'
            if (kit.status == 'elder'):
                kit.moons = randint(120, 150)
            if randint(0, 5) == 0 and kit.backstory not in ['medicine_cat1', 'medicine_cat2', 'disgraced']:  # chance to keep name
                # kit.name.prefix = choice(names.loner_names)
                kit.name.suffix = ''
            elif randint(0, 3) == 0 and kit.backstory not in ['medicine_cat1', 'medicine_cat2', 'disgraced']:
                kit.name.prefix = choice(names.loner_names)
                kit.name.suffix = choice(names.normal_suffixes)
